...
public class Main {
	...
	private static void intialiseDatabase() throws ClassNotFoundException, SQLException {
		if (!new DatabaseManager(ConfigLoader.getUrl(), ConfigLoader.getUser(), ConfigLoader.getPass())
				.tableExists(ConfigLoader.getFoodTableName(), ConfigLoader.getDbName())) {
			// create the table

			// fields are name, ean, and for each base there are 3 fields
			int length = (DatabaseManager.tertiaryFieldNameBases.length * 3)
					+ (DatabaseManager.binaryFieldNameBases.length * 2) + DatabaseManager.continuousFieldNames.length
					+ 2;
			...
			new DatabaseManager(ConfigLoader.getUrl(), ConfigLoader.getUser(), ConfigLoader.getPass())
					.createTable(ConfigLoader.getFoodTableName(), fieldNames, fieldTypes);
		} else {
